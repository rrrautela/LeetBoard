...
// Estimated total user count for rank display
const TOTAL_USERS = 20000000;
const TOTAL_EASY_PROBLEMS = 900;
const TOTAL_MEDIUM_PROBLEMS = 1800;
const TOTAL_HARD_PROBLEMS = 700;
const TOTAL_ALL_PROBLEMS =
  TOTAL_EASY_PROBLEMS + TOTAL_MEDIUM_PROBLEMS + TOTAL_HARD_PROBLEMS;
...
// Progress Circle: colored arc, always horizontal text, responsive font
...
function ParticipantCard({ p, idx }) {
  ...
  return (
    ...
    >
      {/* Rank */}
      <div
        className={`flex items-center justify-center w-full sm:w-16 flex-shrink-0 ${rankBgColor} rounded-lg py-2 sm:py-0`}
      >
        {isTop1 || isTop2 || isTop3 ? (
          <MedalIcon className={`w-8 h-8 ${rankTextColor}`} />
        ) : (
          <div className={`text-2xl font-extrabold ${rankTextColor}`}>
            {idx + 1}
          </div>
        )}
      </div>
      ...
      <img
        ...
        onError={(e) => {
          ...
          e.target.src = `https://placehold.co/64x64/333/FFF?text=${p.username
            .charAt(0)
            .toUpperCase()}`;
        }}
      />
      ...
      <div className="flex justify-center sm:justify-end items-center gap-6 mt-4 sm:mt-0 flex-wrap">
        <ProblemStatCircle
          label="Easy"
          solved={p.easySolved}
          total={TOTAL_EASY_PROBLEMS}
          strokeColor="#22c55e"
        />
        <ProblemStatCircle
          label="Medium"
          solved={p.mediumSolved}
          total={TOTAL_MEDIUM_PROBLEMS}
          strokeColor="#eab308"
        />
        <ProblemStatCircle
          label="Hard"
          solved={p.hardSolved}
          total={TOTAL_HARD_PROBLEMS}
          strokeColor="#ef4444"
        />
        <ProblemStatCircle
          label="Total"
          solved={p.problemsSolved}
          total={TOTAL_ALL_PROBLEMS}
          strokeColor="#3b82f6"
        />
      </div>
    </a>
